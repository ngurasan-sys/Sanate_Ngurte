...
interface MarketState {
  indices: Partial<Record<'NIFTY' | 'SENSEX', MarketIndex>>;
  selectedSymbol: 'NIFTY' | 'SENSEX';
  timeframe: '5m' | '15m' | '30m' | '1h' | 'Daily';
  setIndices: (indices: Partial<Record<'NIFTY' | 'SENSEX', MarketIndex>>) => void;
...
export const useMarketStore = create<MarketState>((set) => ({
  indices: {} as Partial<Record<'NIFTY' | 'SENSEX', MarketIndex>>,
